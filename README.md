...
<br>
예를들어 어떤 플레이어가 게임시작! 버튼을 눌러서 큐를 돌리면 이 사람을 대기자큐에 집어넣는다. 그뒤 다른 플레이어가 게임시작! 버튼을 눌러서 큐를 돌리면 대기자큐에서 하나 빼서 서로 매치시켜주는 방법이 있겠다. <br>
<br>
하지만 이 방법으로는 문제를 해결할수 없다. 만약 큐를 돌리던 플레이어가 다시 게임시작! 버튼을 눌러서 닷지한다면? 그렇다면 대기자큐에서 이 플레이어를 어떻게 제거할까. 대기자큐엔 본인 말고 다른 플레이어들도 순서대로 들어있을 텐데? <br>
<br>
